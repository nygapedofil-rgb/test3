...
if __name__ == '__main__':
    ...
    if download:
        try:
            with requests.post(url1, stream=True, headers={'user-Agent': 'mozilla'}, json={"filename": f'{download}',"username":f'{username}',"password":f'{password}'}) as response:
        ...
        try:
            with open(f'{upload}', "rb") as f:
                ...
                response = requests.post(url, files=files,data={"username":f'{username}',"password":f'{password}'})
